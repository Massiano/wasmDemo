...
export function computeSingleFreq(
  ...
  // DIAGNOSTIC 1: Check Global State
  trace("DIAG: FreqIndex", 1, <f64>freqIndex);
  trace("DIAG: SampleRate", 1, <f64>sampleRate);
  trace("DIAG: nSamples", 1, <f64>nSamples);
  ...
  // DIAGNOSTIC 2: Check Scale/Support
  trace("DIAG: Scale", 1, scale);
  trace("DIAG: Support", 1, <f64>support);

  const rowOffset: usize = CWT_START + <usize>(freqIndex * nTimeOut) * 4;
  trace("DIAG: RowOffset", 1, <f64>rowOffset);

  let maxMag: f32 = 0.0;
  let sampleCheckPerformed: boolean = false;
  ...
  for (let ti: u32 = 0; ti < nTimeOut; ti++) {
    ...
    for (let dt: i32 = -support; dt <= support; dt++) {
      ...
      // DIAGNOSTIC 3: Check if we are actually reading audio (once per frequency)
      if (!sampleCheckPerformed && x != 0) {
          trace("DIAG: Found Non-Zero Audio at t", 1, <f64>t);
          trace("DIAG: Sample Value", 1, x);
          sampleCheckPerformed = true;
      }
  ...
  trace("DIAG: Final maxMag", 1, <f64>maxMag);
  return maxMag;
}
